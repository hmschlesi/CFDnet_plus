def VTK2np(path_to_main, folders, xres:int, yres:int, x_lim:float, y_lim:float):
    ...
    for i in path_to_folder:
        ...
        for j in files:
            # this part needs to dynamic with field variable
            mesh=meshio.read(j)
            #print(j)
            points = mesh.points
            p_t=mesh.point_data['p']
            U_t=mesh.point_data['U']
            nut_t=mesh.point_data['nut']
            nuTilda_t=mesh.point_data['nuTilda']
            ...
            p_new=p_t[boolArr]
            U_new=U_t[boolArr]
            nut_new=nut_t[boolArr]
            nuTilda_new=nuTilda_t[boolArr]
            ...
            nut_grid_new = griddata(points_new, nut_new, (grid_x, grid_y), method='nearest')
            nuTilda_grid_new = griddata(points_new, nuTilda_new, (grid_x, grid_y), method='nearest')
            p_list.append(p_grid_new)
            U_list.append(U_grid_new)
            nut_list.append(nut_grid_new)
            nuTilda_list.append(nuTilda_grid_new)
       
        p=np.array(p_list)
        nut=np.array(nut_list)
        nuTilda=np.array(nuTilda_list)
        [a,b,c]=p.shape
        print(p.shape)
        U=np.array(U_list)
        p=p.reshape(a,b,c,1)
        nut=nut.reshape(a,b,c,1)
        nuTilda=nuTilda.reshape(a,b,c,1)
        df=np.concatenate((U, p,nut,nuTilda), axis=3)
        print(df.shape)
        [a,b,c,d]=df.shape
...
def model2OF(prediction):
    print('model to Openfoam field data')
...
def extractInput2d(mainfolder : str, fields :str, xlim, ylim, xres, yres, ycut):
    import numpy as np
    import os
    from scipy.interpolate import griddata
    import re
